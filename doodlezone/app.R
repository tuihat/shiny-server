...
ui <- dashboardPage(
  ...
  body = dashboardBody(
    ...
    fluidRow(
      box(width = 3,
          plotOutput("plot_APCdist")
      ),
      box(width = 3,
          plotOutput("plot_HLAPCdist")
      ),
      box(width = 3,
          plotOutput("plot_XCBdist")
      ),
      box(width = 3,
          plotOutput("plot_RCBdist")
      )
    ),
...
#########---Output Graphs---#####################################################   
  ...
  output$plot_TotalCoredist <- renderPlot({
    ggplot() +
      geom_histogram(data = chosen_progs(), aes(x = Total.cores..no..),
                     binwidth = 0.01*max(chosen_progs()$Total.cores..no..), fill = "gold", color = "gray40") +
      labs(x = "Total Cores per Hole", subtitle = paste0("Total Cores by Hole Distribution, n = ", sum(!is.na(chosen_progs()$Total.cores..no..))))
  })
  ###################################
  ...
  ###################################
  output$plot_APCdist <- renderPlot({
    ggplot() +
      geom_histogram(data = chosen_progs(), aes(x = APC.cores..no..),
                     binwidth = 0.01*max(chosen_progs()$APC.cores..no..), fill = "mediumturquoise", color = "gray40") +
      labs(x = "APC Cores per Hole", subtitle = paste0("APC Coring Distribution, n = ", sum(!is.na(chosen_progs()$APC.cores..no..)))) +
      scale_y_log10()
  })
  ###################################
  output$plot_HLAPCdist <- renderPlot({
    ggplot() +
      geom_histogram(data = chosen_progs(), aes(x = HLAPC.cores..no..),
                     binwidth = 0.01*max(chosen_progs()$HLAPC.cores..no..), fill = "royalblue4", color = "gray40") +
      labs(x = "HLAPC Cores per Hole", subtitle = paste0("HLAPC Coring Distribution, n = ", sum(!is.na(chosen_progs()$HLAPC.cores..no..)))) +
      scale_y_log10()
  })
  ###################################
  output$plot_XCBdist <- renderPlot({
    ggplot() +
      geom_histogram(data = chosen_progs(), aes(x = XCB.cores..no..),
                     binwidth = 0.01*max(chosen_progs()$XCB.cores..no..), fill = "palevioletred3", color = "gray40") +
      labs(x = "XCB Cores per Hole", subtitle = paste0("XCB Coring Distribution, n = ", sum(!is.na(chosen_progs()$XCB.cores..no..)))) +
      scale_y_log10()
  })
  ###################################
  output$plot_RCBdist <- renderPlot({
    ggplot() +
      geom_histogram(data = chosen_progs(), aes(x = RCB.cores..no..),
                     binwidth = 0.01*max(chosen_progs()$RCB.cores..no..), fill = "orange", color = "gray40") +
      labs(x = "RCB Cores per Hole", subtitle = paste0("RCB Coring Distribution, n = ", sum(!is.na(chosen_progs()$RCB.cores..no..)))) +
      scale_y_log10()
  })
}
